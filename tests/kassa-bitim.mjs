// =============================================================
//  CLARY — BITIM VA TO'LOV SINOVI
//
//  Bu yerda tekshiriladigan narsa "xato" emas, JIM xato: qarz
//  noto'g'ri tomonga yozilsa, ekranda hammasi joyida ko'rinadi va
//  faqat oy oxirida «hisob to'g'ri kelmayapti» bo'lib chiqadi.
//
//  Eng muhim uchtasi:
//
//   1. TOVAR KASSAGA TEGMAYDI. Eski ilovada «tovar berdim» chiqim
//      bo'lib kassadan pul yechardi — qarz ham, xarajat ham bo'lib
//      ikki marta hisoblanardi.
//   2. ISHORA QOIDASI. 'berdim' -> +summa, 'oldim' -> -summa;
//      bitim ham, to'lov ham AYNAN shu qoida bilan qo'shiladi.
//   3. TASDIQ QOLDIQQA TA'SIR QILMAYDI. Tasdiqlanmagan bitim ham
//      qoldiqda turadi — aks holda daftar hamkorning tugma
//      bosishiga bog'liq bo'lib qolardi.
//
//  Hammasi BITTA `do` blokida bajariladi va oxirida `raise
//  exception` bilan QAYTARIB OLINADI — jonli bazada iz qolmaydi.
//
//  Ishga tushirish:  node tests/kassa-bitim.mjs
//  SQL ni ko'rish:   node tests/kassa-bitim.mjs --sql
//                    (Dashboard -> SQL Editor ga qo'yish uchun)
// =============================================================
...
console.log('\n\x1b[1mCLARY — BITIM VA TO‘LOV\x1b[0m');
...
if (!K?.mgmt_token) {
  console.log('\n  \x1b[33m!\x1b[0m kodchi/kalitlar.json topilmadi — sinov o‘tkazib yuborildi.');
  console.log('    SQL ni ko‘rish: node tests/kassa-bitim.mjs --sql\n');
  process.exit(0);
}
...
const qoldi = await sqlXom(
  "select count(*) as n from public.organizations where name like 'SINOV-BITIM%'",
);
let soni = -1;
try {
