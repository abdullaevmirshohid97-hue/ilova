-- =============================================================
--  IMPORT: ishlab chiqarilgan sana ham yoziladi
--
--  Foydalanuvchi yuklaydigan prixod fakturalarida seriya, ishlab
--  chiqarilgan sana va yaroqlilik muddati bor. Robot endi uchalasini
--  ham o'qiydi; bu yerda ular bazaga tushadi:
--   * partiya yozuvi (dori_batches) — seriya + ikkala sana bilan;
--   * katalog kartochkasi (dori_products) — oxirgi ma'lum sana bilan.
--
--  Fakturaga esa partiya bo'yicha chiqadi (FEFO), ya'ni mijoz aynan
--  o'ziga ketayotgan partiyaning ma'lumotini ko'radi.
-- =============================================================
...
begin
  ...
  loop
    ...
    if v_id is null then
      insert into dori_products (barcode, name, name_norm, manufacturer, grp, unit,
                                 price, stock, made_at, last_import)
      values (r.barcode, r.name, r.name_norm, r.manufacturer, r.grp, r.unit,
              r.price, r.stock, r.made_at, v_import)
  ...
  -- Partiyalar: bir xil (dori + seriya + muddat) fayl ichida bir necha
  -- marta uchrashi mumkin, shuning uchun avval guruhlanadi
