-- =============================================================
-- YUKCHIBOLLA — mijoz katalogni ko'rayotganda ham menejer narxini
-- ko'rsin (avval faqat buyurtma berilgandan KEYIN to'g'ri narx
-- hisoblanardi — CatalogScreen/CartScreen `prices` jadvalidan
-- to'g'ridan-to'g'ri o'qib, faqat baza narxni ko'rsatardi, chunki
-- mijozning manager_prices/manager_customer_prices'ga o'qish huquqi
-- yo'q edi va bo'lmasligi ham kerak — bu boshqa menejerlarning
-- narxini oshkor qilib qo'yardi).
--
-- my_effective_prices() — SECURITY DEFINER, faqat chaqiruvchi
-- mijozning O'ZIGA tegishli yakuniy narxlarni qaytaradi (create_order
-- bilan bir xil ustuvorlik: mijozga xos > menejerning umumiy narxi >
-- baza narx), manager_prices jadvalining o'ziga to'g'ridan-to'g'ri
-- kirish ochilmaydi.
-- =============================================================

create or replace function public.my_effective_prices()
returns table(variant_id uuid, price numeric(14,0))
